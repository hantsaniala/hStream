...
func HandlePrepareVideoDownloadTask(ctx context.Context, t *asynq.Task) error {
	...
	video.GenMetadata(filepath.Join(destDir, "metadata-playlist.json"), string(playlistData))
	video.GenMetadata(filepath.Join(destDir, "metadata.json"), string(videoData))
